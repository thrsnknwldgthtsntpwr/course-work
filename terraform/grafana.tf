resource "yandex_compute_disk" "grafana-disk" {
  name = "grafana-disk"
  type = "network-hdd"
  zone = "ru-central1-a"
  size = "10"
  image_id = "fd833v6c5tb0udvk4jo6"
}

resource "yandex_compute_instance" "grafana" {
  name = "grafana"
  allow_stopping_for_update = true
  platform_id = "standard-v3"
  zone = "ru-central1-a"

  resources {
    cores  = "2"
    memory = "1"
    core_fraction = "20"
  }

  boot_disk {
    disk_id = yandex_compute_disk.grafana-disk.id
  }

  scheduling_policy {
    preemptible = true
  }

  network_interface {
    subnet_id = yandex_vpc_subnet.private-subnet-a.id
    security_group_ids = [ yandex_vpc_security_group.ssh.id, yandex_vpc_security_group.grafana.id, yandex_vpc_security_group.output.id ]
    nat = true
  }

  metadata = {
    ssh-keys = "thrsnknwldgthtsntpwr:ssh-rsa  thrsnknwldgthtsntpwr@ubnt"
    user-data = "${file("meta.txt")}"
  }
}

output "grafana-public-ip-address" {
  value = yandex_compute_instance.grafana.network_interface.0.nat_ip_address
}
output "grafana-private-ip-address" {
  value = yandex_compute_instance.grafana.network_interface.0.ip_address
}
